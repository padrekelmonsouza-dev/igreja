...
import { MISSION_PATHS, MISSION_TOPICS, type MissionPath, type MissionTopic } from "../data/missions";

type ModalItem =
  | { kind: "path"; data: MissionPath }
  | { kind: "topic"; data: MissionTopic };

function MissaoModal({ item, onClose }: { item: ModalItem; onClose: () => void }) {
  const titleId = useId();
  const closeRef = useRef<HTMLButtonElement>(null);
  const isPath = item.kind === "path";
  const title = item.data.title;
  const kicker = isPath ? item.data.kicker : "Missões";
...
function PathCard({ path, onOpen }: { path: MissionPath; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
...
function TopicCard({ topic, onOpen }: { topic: MissionTopic; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
  ...
  return (
    <>
      <PageHero
        ...
        title="Missões"
        intro="A missão ortodoxa anuncia Cristo, planta comunidades e forma discípulos na Tradição da Igreja."
        crumbs={[{ href: "/missoes", label: "Missões" }]}
      />

      <section id="caminhos" className="site-section mx-auto w-full max-w-[1280px] px-4">
        <div>
          <p className="kicker">A missão da Igreja</p>
          <h2 className="mt-3 font-serif text-4xl">Anunciar, plantar e formar.</h2>
          <p className="mt-3 max-w-3xl text-stone">
            O sentido da missão, a presença no Brasil, as comunidades e o modo de ajudar. Cada card abre a
            explicação completa.
          </p>
        </div>
      ...
      <section id="temas" className="site-section mx-auto w-full max-w-[1280px] px-4">
        <div>
          <p className="kicker">Caminhos da missão</p>
          <h2 className="mt-3 font-serif text-4xl">Como a Igreja vai ao encontro.</h2>
          <p className="mt-3 max-w-3xl text-stone">
            Testemunho, cidades sem paróquia, catequese, comunidades, vocação e o sustento da missão.
          </p>
        </div>
      ...
      <KnowOrthodoxy />
      {open ? <MissaoModal item={open} onClose={() => setOpen(null)} /> : null}
    </>
  );
}
