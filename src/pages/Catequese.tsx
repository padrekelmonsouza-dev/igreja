...
import {
  ...
  CATECHESIS_TOPICS,
  type CatechesisPath,
  type CatechesisTopic,
} from "../data/catechesis";

type ModalItem =
  | { kind: "path"; data: CatechesisPath }
  | { kind: "topic"; data: CatechesisTopic };

function CatequeseModal({ item, onClose }: { item: ModalItem; onClose: () => void }) {
  const titleId = useId();
  const closeRef = useRef<HTMLButtonElement>(null);
  const isPath = item.kind === "path";
  const title = item.data.title;
  const kicker = isPath ? item.data.kicker : "Catequese";
...
function PathCard({ path, onOpen }: { path: CatechesisPath; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
    ...
    >
      ...
      <div className="relative mt-auto p-5 text-white sm:p-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-gold-soft">{path.kicker}</p>
        <h3 className="mt-1 font-serif text-[1.35rem] leading-tight sm:text-2xl">{path.title}</h3>
        <p className="mt-3 text-sm leading-6 text-ivory/90">{path.summary}</p>
      </div>
    </button>
  );
}

function TopicCard({ topic, onOpen }: { topic: CatechesisTopic; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
  ...
  return (
    <>
      <PageHero
        ...
        title="Catequese"
        intro="Caminhos para conhecer a fé ortodoxa com clareza, respeito e profundidade."
        crumbs={[{ href: "/catequese", label: "Catequese" }]}
      />

      <section id="caminhos" className="site-section mx-auto w-full max-w-[1280px] px-4">
        <div>
          <p className="kicker">Caminhos da fé</p>
          <h2 className="mt-3 font-serif text-4xl">Como a Igreja inicia.</h2>
          <p className="mt-3 max-w-3xl text-stone">
            Iniciação, doutrina, Batismo e vida na comunidade. Cada card abre a explicação completa.
          </p>
        </div>
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {CATECHESIS_PATHS.map((path) => (
            <PathCard key={path.slug} path={path} onOpen={() => setOpen({ kind: "path", data: path })} />
          ))}
        </div>
      </section>
      ...
      <section id="temas" className="site-section mx-auto w-full max-w-[1280px] px-4">
        <div>
          <p className="kicker">Temas da catequese</p>
          <h2 className="mt-3 font-serif text-4xl">O que se aprende na Igreja.</h2>
          <p className="mt-3 max-w-3xl text-stone">
            Cristo, a Igreja, os Santos Mistérios, a oração e o primeiro passo até a comunidade.
          </p>
        </div>
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {CATECHESIS_TOPICS.map((topic) => (
      ...
      <KnowOrthodoxy />
      {open ? <CatequeseModal item={open} onClose={() => setOpen(null)} /> : null}
    </>
  );
}
